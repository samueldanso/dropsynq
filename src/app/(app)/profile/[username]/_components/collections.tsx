...
export function Collections({ address }: CollectionsProps) {
	...
	if (isError) return <div className="p-4 text-destructive">Failed to load collections.</div>;

	const collections = balances?.filter((balance) => balance.coin && balance.coin.owner !== address);
