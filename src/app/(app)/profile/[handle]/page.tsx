...
export default function ProfilePage() {
	...
	const coins = useQuery({
		...
		queryFn: async () => {
			...
			// Only include coins that are music/audio (audio/mpeg or category: music)
			return (
				res?.data?.profile?.createdCoins?.edges
					?.map((edge: { node: ZoraCoin }) => edge.node)
					?.filter((coin: ZoraCoin) => {
						const mimeType = coin.mediaContent?.mimeType;
						const isAudio = mimeType === "audio/mpeg";
						const hasAudioFile =
							typeof coin.mediaContent?.originalUri === "string" &&
							/\.(mp3|wav|ogg)$/i.test(coin.mediaContent.originalUri);
						return isAudio || hasAudioFile;
					}) || []
			);
		},
		enabled: !!identifier,
